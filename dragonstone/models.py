...
class RecruitmentSubmission(DragonstoneBaseSubmission):
    ...
    def annotate_dragonstone_pts(cls, account=None):
        ...
        Only consider submission made within the last 3 months.
        """
        dragonstone_pts = cls.objects.accepted().filter(date__gte=expiration_period).annotate(
...
class PVMSplitSubmission(DragonstoneBaseSubmission):
    ...
    def annotate_dragonstone_pts(cls, account=None):
        ...
        Only consider submission made within the last 3 months.
        """
        dragonstone_qs = cls.objects.accepted().filter(date__gte=expiration_period).annotate(
            dragonstone_pts=Case(
...
class MentorSubmission(DragonstoneBaseSubmission):
    ...
    def annotate_dragonstone_pts(cls, account=None):
        ...
        Only consider submission made within the last 3 months.
        """
        dragonsone_qs = cls.objects.accepted().filter(date__gte=expiration_period).annotate(
            dragonstone_pts=Case(
...
class EventSubmission(DragonstoneBaseSubmission):
    ...
    def annotate_dragonstone_pts(cls, account=None):
        ...
        Only consider submission made within the last 3 months.
        """
        hosts_qs = cls.objects.accepted().filter(date__gte=expiration_period).annotate(
            dragonstone_pts=Case(
        ...
        participants_qs = cls.objects.accepted().filter(date__gte=expiration_period).annotate(
            dragonstone_pts=Case(
        ...
        donors_qs = cls.objects.accepted().filter(date__gte=expiration_period).annotate(
            dragonstone_pts=Case(
